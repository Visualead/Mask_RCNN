...
class DavisDataset(utils.Dataset):
    def load_davis(self, dataset_dir, subset, class_ids=None,
        ...
        anns = {}
        image_filename = []
        davis_train_list_path = os.path.join(dataset_dir,'Augmentation/train_480p_2016_aug_light_list.txt')
        with open(davis_train_list_path) as f:
            for ind,i in enumerate(f.readlines()):
                ii = i.split(' ')
                ann = ii[0]
                img = ii[1]
                anns[ind] = ann
                image_filename.append(img)

        image_ids = range(len(anns))

        # Add images
        for i in image_ids:
            self.add_image(
                "davis", image_id=i,
                path=os.path.join(dataset_dir, image_filename[i]),
                width=image_width,
                height=image_height,
                #annotations - dictionary {id: 'path-to-annotation'}
                annotations=anns)
        # if return_coco:
        #     return coco
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        dataset_train.load_davis(args.dataset, "train")
        dataset_train.load_davis(args.dataset, "val35k")
        dataset_train.prepare()

        # Validation dataset
        dataset_val = DavisDataset()
        dataset_val.load_davis(args.dataset, "minival")
        dataset_val.prepare()
